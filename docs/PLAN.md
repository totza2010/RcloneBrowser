...
| §6.8 | **เก็บทุกอย่างลง DB + ประวัติการรัน** | ✅ | = S13 · V-21 |
| §6.9 | **Extension — teldrive integrity check** | ⬜ **ใหม่** | = S14 · ทำหลัง S13 |
| §6.3 E1 | `--run-task` headless | ✅ | V-17 **ผ่านเบื้องต้น** |
...
| เรื่อง | ที่มาร์คไว้ |
|---|---|
| **แก้ชนิดการถ่ายโอนของ task ที่มีอยู่** (upload ↔ download) | ขอเพิ่ม 2026-08-24 · ดู §6.10 |
| **ตารางเวลาหนึ่งอัน ทำได้หลาย task** | ขอเพิ่ม 2026-08-24 · ทำหลัง S4 · ดู §6.11 |
...
> **ทำเสร็จ 2026-08-09** ([`VERIFY.md` V-21](VERIFY.md))

### สถานะ ณ 2026-08-09

| ส่วน | สถานะ | ไฟล์ |
|---|---|---|
| ฐานข้อมูล + schema + migration + WAL/busy timeout | ✅ | `src/database.*` |
| ประวัติการรัน (เขียน/อ่าน/ตัดข้อมูลเก่า) | ✅ | `src/run_history.*` |
| ทุกงานใน `JobRegistry` เขียนประวัติ | ✅ | `src/job_registry.cpp` |
| `--run-task` เขียนประวัติ (`transfer_mode = cli`) | ✅ | `src/task_runner.cpp` |
| ไดรเวอร์ SQLite ใน CI และในอิมเมจ Docker | ✅ | `ci.yml` · `docker/Dockerfile` |
| ย้าย `tasks.bin` / `queue.conf` / `scheduler.conf` เข้า DB | ✅ | `list_of_job_options.cpp` · `src/config_store.*` · นำเข้าครั้งเดียวแล้วเปลี่ยนชื่อเป็น `.migrated` |
| ทำงานต่อได้เมื่อ**ไม่มีไดรเวอร์ SQLite** (อ่าน/เขียนไฟล์เดิมเหมือนเดิม) | ✅ | ทางถอย — ไม่มี DB ไม่ได้แปลว่า task หาย |
| **History** เป็นแท็บย่อยในหน้า Jobs (ดู/กรอง/เปิด log/ล้าง) | ✅ | `src/history_widget.*` |

**แยกเป็นสองรอบโดยตั้งใจ** — ของที่หายจริงคือประวัติ ส่วน `tasks.bin` และ `.conf`
สองไฟล์ *ไม่ได้หาย* แค่รูปแบบไม่ดี การย้ายจึงเป็นการปรับปรุง ไม่ใช่การกู้ของที่หาย
และมีความเสี่ยงต่อข้อมูลจริงของผู้ใช้มากกว่ามาก
...
1. ✅ ปิดแล้วเปิดโปรแกรมใหม่ → ประวัติงานที่รันไปแล้วยังอยู่ครบ พร้อมลิงก์ไปไฟล์ log
   *(เขียนครบแล้ว · ข้อ "เปิดใหม่แล้วยังอยู่" รอผู้ทดสอบยืนยัน — V-21 ข้อ 1)*
2. ✅ `tasks.bin` / `queue.conf` / `scheduler.conf` เดิมถูกย้ายเข้า DB ครบ **โดยไม่สูญหาย**
   *(ลองกับข้อมูลจริงของผู้ใช้แล้ว 2026-08-09: 6 task + 1 scheduler ครบ ไฟล์เดิมเปลี่ยนชื่อเป็น `.migrated`)*
3. ✅ รัน `--run-task` จาก CLI ขณะเปิดหน้าต่างอยู่ → **ทั้งสองเขียนประวัติได้ ไม่ชนกัน**
   *(WAL + busy timeout · test เขียนพร้อมกันสองการเชื่อมต่อผ่าน · รอลองจริง — V-21 ข้อ 6)*
4. ✅ test ที่ลิงก์แค่ `rbcore` ครอบ: สร้าง/อ่าน/ย้ายข้อมูล/เขียนพร้อมกันสองการเชื่อมต่อ
   *(`test_run_history` 16 เคส · `test_config_store` 8 เคส · `test_task_store` +2 เคส)*

---

## 6.11 ตารางเวลาหนึ่งอัน ทำได้หลาย task

> ขอเพิ่ม 2026-08-24 จากการใช้งานจริง · **ทำหลังงานย้ายคิว (S3/S4) เสร็จ**

**ตอนนี้ผูกได้ task เดียว** — `Schedule::taskId` เป็นค่าเดี่ยว ถ้าอยากให้ตี 2 ทำสามอย่าง
ต้องสร้างตารางเวลาสามอัน เวลาเดียวกัน แล้วดูแลทั้งสามให้ตรงกันเอง

### สิ่งที่ต้องตัดสินก่อนเขียนโค้ด

| คำถาม | ทำไมสำคัญ |
|---|---|
| **หลาย task ทำพร้อมกัน หรือทีละอัน** | ถ้าพร้อมกัน = แย่งแบนด์วิดท์และอาจชนปลายทางเดียวกัน · ถ้าทีละอัน = คือการส่งเข้าคิว ซึ่งมีอยู่แล้ว |
| **ลำดับสำคัญไหม** | ถ้าสำคัญ ต้องเก็บ position เหมือน `queue_entry` |
| **`executionMode` ที่มีอยู่** | มีสองค่าอยู่แล้ว: รันทันที / ต่อคิว · **ค่า "ต่อคิว" คือคำตอบของ "ทีละอัน" อยู่แล้ว** ไม่ต้องประดิษฐ์กลไกใหม่ |
| **task ตัวหนึ่งถูกลบ** | ตารางเวลาควรทำที่เหลือต่อ ไม่ใช่หยุดทั้งอัน (กฎเดียวกับที่คิวใช้อยู่) |
| **`mRequestId` เดี่ยว** | ตอนนี้ตารางเวลาจำ requestId ได้ครั้งละหนึ่ง · หลาย task = หลาย requestId ต่อหนึ่งรอบ ซึ่งกระทบการรายงานสถานะกลับ |

### รูปร่างข้อมูล

`schedule.task_id` เป็นคอลัมน์เดี่ยว — ทางที่ถูกคือ **ตารางเชื่อม** ไม่ใช่ยัดหลาย id ลงช่องเดียว:
...
**= schema v3** · การย้ายข้อมูลตรงไปตรงมา: ตารางเวลาเดิมหนึ่งอัน → หนึ่งแถว position 0
และ `schedule.task_id` เก็บไว้เป็นตัวแรกเพื่อให้ build เก่าอ่านแล้วยังทำงานได้

### ทำไมต้องรอ S4 ให้เสร็จก่อน

`SchedulerWidget` ยังถือ state 23 ฟิลด์ของตัวเองและเป็นคนตัดสินว่าถึงเวลาหรือยัง
**ถ้าเพิ่มหลาย task ตอนนี้ จะเพิ่มความซับซ้อนเข้าไปในที่ที่กำลังจะย้ายออกพอดี**

`SchedulerStore` (L1) ที่ทำไว้แล้วรับเรื่องนี้ได้ไม่ยาก — `Schedule` เปลี่ยนจาก
`QString taskId` เป็น `QStringList taskIds` และ `due()` คืนรายการงานที่ต้องเริ่ม
ส่วน widget เหลือหน้าที่แสดงผลอย่างเดียว

### ขอบเขต

`schedule.*` · `scheduler_store.*` · `scheduler_widget.*` · schema v3
**ไม่กระทบ** คิว งาน หรือประวัติ

---

## 6.10 แก้ชนิดการถ่ายโอนของ task ที่มีอยู่

> ขอเพิ่ม 2026-08-24 จากการใช้งานจริง

**ตอนนี้เปลี่ยนไม่ได้** — `JobOptions::jobType` (Upload / Download) ถูกกำหนดตอนกด Add
และ `TransferDialog` เปิดโหมดแก้ไขด้วยชนิดเดิมเสมอ ถ้าตั้งผิดต้องลบ task แล้วสร้างใหม่
**ซึ่งทำให้เสียประวัติการรันของมันไปด้วย** เพราะ S13 ผูกประวัติกับ `task_id`

**สิ่งที่ต้องตัดสินก่อน ไม่ใช่แค่เปิดช่องให้แก้:**

- `source` กับ `dest` สลับด้านกันเมื่อชนิดเปลี่ยน — สลับให้อัตโนมัติ หรือให้กรอกใหม่?
- ตัวเลือกบางตัวมีความหมายกับทางเดียว (เช่น `--drive-shared-with-me` ฝั่ง remote)
- task ที่อยู่ในคิวหรือมีตารางเวลาผูกอยู่ ควรแก้ได้ไหมขณะรออยู่
- mount ไม่ใช่ upload/download จึงไม่เกี่ยวกับเรื่องนี้

**ขอบเขต:** `TransferDialog` (L3) กับ `JobOptions` (L0) เท่านั้น ไม่กระทบการย้ายคิว
ทำแยกได้ตอนไหนก็ได้

---

## 6.9 Extension — ความสามารถเฉพาะ backend โดยไม่ให้รั่วเข้า core

> เพิ่มเข้าแผน 2026-08-08 · ระบบ **S14** ใน [`API.md` §12](API.md)
> ตัวแรกที่จะทำคือ **ตรวจความครบของไฟล์บน teldrive**

### ปัญหาที่ทำให้ต้องมีเรื่องนี้

เจอจากการใช้งานจริง (2026-08-08): ไฟล์ขนาด 4.6G ที่ **rclone ส่งครบ 10 ก้อนและได้รับ
คำยืนยันครบ 10 ก้อน** แต่เซิร์ฟเวอร์เก็บจริงแค่ 8 ก้อน ขาดไป ≥653M

ตรวจ log ของงานนั้นแล้ว (`20260808-004143-copy-4324eb58.log`):

```
Sending chunk       10   ครบ
Done sending chunk  10   ครบ
Multi-thread Copied (new)
```

**ฝั่ง client ไม่มีทางรู้เลย** — ได้ 200 กลับมาแล้ว งานประกาศสำเร็จ ขนาดในรายการก็ถูก
จะรู้ก็ต่อเมื่อเอาเครื่องมือภายนอกมาส่อง

### ทำไม `rclone check` ช่วยไม่ได้

| ตรวจแล้ว (2026-08-08) | ผล |
|---|---|
| `rclone backend features tgdrive_adult_01:` | **`Hashes: []`** |
| `--teldrive-hash-enabled` | มีอยู่จริง · **ค่าเริ่มต้น true** · "Enable Blake3 Tree Hashing" |
| ฟิลด์ `hash` ใน `api.FileInfo` ของ backend | **มีอยู่** |

> ⚠️ **แก้บันทึกเดิม** — [`DEV-SETUP.md`](DEV-SETUP.md) เขียนว่า teldrive ไม่มี hash เลย
> ความจริงคือ **teldrive คำนวณ blake3 และเก็บไว้ใน API แต่ไม่ได้ประกาศให้ rclone**
> `rclone check` จึงเทียบได้แค่ขนาด ซึ่งเป็นค่าที่ถูกต้องอยู่แล้วในเคสนี้ — จับไม่ได้
> เป็นคนละเรื่องกับ "ไม่มี hash" และความต่างนี้คือเหตุผลทั้งหมดที่ต้องมี extension

### ทำไมต้องเป็น extension ไม่ใช่ใส่เข้า core

กฎข้อ 1 ของ [`ARCHITECTURE.md`](ARCHITECTURE.md) ห้าม L0/L1 รู้จัก GUI · ในทำนองเดียวกัน
**L0/L1 ไม่ควรรู้จัก backend ตัวใดตัวหนึ่งเป็นพิเศษ** — `RcloneCapabilities` ที่ทำไว้แล้ว
ถามความสามารถแบบทั่วไปจาก rclone โดยไม่มีชื่อ backend อยู่ในโค้ดเลย ซึ่งเป็นเหตุผลที่มันยัง
ใช้ได้กับ backend ที่ยังไม่เกิด

teldrive เป็นจุดขายของ fork นี้ก็จริง แต่ถ้าเอา endpoint ของมันยัดเข้า core:

- core จะมีความรู้ที่ใช้ได้กับ remote ชนิดเดียว
- คนที่ใช้ rclone mainline ล้วนๆ ต้องแบกโค้ดที่ไม่มีวันทำงาน
- backend ตัวถัดไปที่อยากได้อะไรแบบนี้จะถูกยัดเข้าไปข้างๆ กันจนกลายเป็นกองรวม

### รูปแบบ: โมดูลตอนคอมไพล์ ไม่ใช่ปลั๊กอินตอนรัน

```
src/ext/teldrive/    →  static lib "rbext_teldrive"  (ลิงก์ Qt Core + Network)
cmake -DRB_EXT_TELDRIVE=ON      (ค่าเริ่มต้น ON สำหรับ fork นี้)
```

ที่ **ไม่เลือก** ปลั๊กอินตอนรัน (`QPluginLoader`, `.dll`/`.so`):

| ต้นทุนที่ต้องจ่าย | คุ้มไหมกับ extension ที่มีตัวเดียวและอยู่ใน repo เดียวกัน |
|---|---|
| ต้องคง ABI ให้เสถียร | ไม่ — คอมไพล์พร้อมกันทุกครั้งอยู่แล้ว |
| ต้องมีระบบค้นหา/โหลด/เวอร์ชัน | ไม่ |
| ต้องคิดเรื่องความปลอดภัยของการโหลดโค้ดจากดิสก์ | ไม่ — และเป็นเรื่องใหญ่กว่าที่ได้ |

โมดูลตอนคอมไพล์ยังบังคับขอบเขตได้จริงเหมือน `rbcore`: **ถ้า core ไปอ้างถึงโมดูล
extension จะลิงก์ไม่ผ่าน** ซึ่งเป็นด่านเดียวกับที่ใช้กันอยู่แล้ว

### หน้าตาของส่วนต่อ

```cpp
// L1 -- core รู้จักแค่ interface ไม่รู้จักชื่อ backend
class RemoteExtension {
public:
  virtual ~RemoteExtension() = default;

  // ชนิดของ remote ที่รับผิดชอบ ("teldrive") -- ตรงกับ type ที่
  // rclone listremotes --long คืนมา
  virtual QString remoteType() const = 0;

  // สิ่งที่ extension นี้ทำได้กับ remote ตัวนั้น เอาไปสร้างเมนู/ปุ่มได้
  virtual QList<ExtensionAction> actions() const = 0;
};

class ExtensionRegistry {
  static ExtensionRegistry &instance();
  RemoteExtension *forRemoteType(const QString &type) const;
};
```

`ExtensionAction` เป็นแค่ชื่อ + คำอธิบาย + ตัวเรียก — **ไม่มีอะไรเกี่ยวกับ widget**
เพื่อให้ CLI และ API เรียกได้เท่ากับหน้าต่าง

### extension ตัวแรก: ตรวจความครบของไฟล์ teldrive

**ยืนยันจาก source ของ backend แล้ว** (`tgdrive/rclone` → `backend/teldrive/`):

| | |
|---|---|
| ตั้งค่า | `--teldrive-api-host` · `--teldrive-access-token` (cookie) |
| endpoint ที่ backend ใช้ | `/api/files` · `/api/files/{id}` · `/api/files/delete` · `/api/files/move` · `/api/files/categories` · `/api/auth/session` |
| `api.FileInfo` | `id` `name` `mimeType` `size` `parentId` `type` `updatedAt` **`hash`** |
| `api.Part` | `id` `salt` — และ `parts[]` อยู่ในคำขอสร้าง/แก้ไขไฟล์ |
| chunk size | `--teldrive-chunk-size` ค่าเริ่มต้น **512Mi** |

**การตรวจขั้นแรก (ไม่ต้องคำนวณ hash):**

```
ถ้า  size > จำนวน part × chunk size สูงสุดของ instance
แล้ว ไฟล์นี้เก็บไม่ครบแน่นอน
```

เป็นการพิสูจน์ ไม่ใช่การเดา — ไม่ต้องรู้ว่า instance ตั้ง chunk size เท่าไหร่
ขอแค่ขอบบนก็สรุปได้ **นี่คือวิธีเดียวกับที่เครื่องมือ audit ภายนอกใช้อยู่**

**การตรวจขั้นที่สอง (blake3):** เทียบ `hash` จาก API กับที่คำนวณจากไฟล์ต้นทาง
ตรวจได้แน่นอนกว่า แต่ต้องมี blake3 tree hash แบบเดียวกับ `backend/teldrive/tdhash`
**ยังไม่ทำในรอบแรก** — ขั้นแรกจับเคสที่เจอจริงได้แล้ว และไม่ต้องเพิ่ม dependency

### เอา access token มาจากไหน

**ไม่อ่าน `rclone.conf` เอง** — ให้ rclone บอก:

```bash
rclone config show <remote>
```

เหตุผล: ไฟล์อาจถูกเข้ารหัสด้วย config password ซึ่งเรามีทางส่งอยู่แล้ว
(`UseRclonePassword()` → `RCLONE_CONFIG_PASS`) การไปแกะไฟล์เองแปลว่าต้องทำ
เรื่องนั้นซ้ำอีกที่หนึ่ง และจะพังทันทีที่รูปแบบไฟล์เปลี่ยน

**🔴 access token เป็น credential** — กฎข้อ 5 ของ `ARCHITECTURE.md` ใช้เต็มที่:
ห้ามลง log · ห้ามอยู่ใน args · ห้ามออกทาง API (S11) · ต้องมี test ที่ยืนยัน
เหมือนที่ `test_job_registry` ทำกับ `--drive-token`

### สิ่งที่ extension ตัวนี้ทำได้ (รอบแรก)

| | |
|---|---|
| ตรวจโฟลเดอร์หนึ่ง → รายการไฟล์ที่ **พิสูจน์ได้ว่าสั้น** | ✅ รอบแรก |
| บอกว่าขาดกี่ part และขาดอย่างน้อยกี่ไบต์ | ✅ รอบแรก |
| สั่งอัปโหลดซ้ำเฉพาะไฟล์ที่เสีย | ✅ รอบแรก (สร้าง task ใหม่จากรายการ) |
| ไฟล์ที่ค้างจากงานที่ถูกหยุดกลางคัน | 🟡 ดูจากประวัติใน **S13** ได้ตรงกว่า |
| เทียบ blake3 | ⬜ รอบหลัง |
| ลบ orphan part | ⬜ รอบหลัง — เป็นการเขียน ต้องระวังกว่ามาก |

### เกี่ยวกับ S13 อย่างไร

**S13 กับ S14 ตอบคนละคำถาม และต้องใช้คู่กัน:**

- **S13 (ประวัติ)** บอกว่างานไหนจบแบบ `stopped` → ไฟล์ที่ค้างเป็นผลของการหยุด **อธิบายได้**
- **S14 (extension)** บอกว่าไฟล์ไหนสั้นทั้งที่งาน **จบสมบูรณ์** → เป็นความล้มเหลวเงียบของเซิร์ฟเวอร์

ถ้ามีแต่ S14 จะแยกไม่ออกว่าไฟล์ที่ไม่ครบมาจากการกดหยุดเองหรือของหาย
**ทำ S13 ก่อนจึงถูกลำดับ**

### เสร็จเมื่อ

1. `cmake -DRB_EXT_TELDRIVE=OFF` build ผ่านและแอปทำงานปกติ (แค่ไม่มีเมนูตรวจ)
2. core ไม่มีคำว่า `teldrive` อยู่เลย — ตรวจด้วย grep ใน `scripts/check_layers.py`
3. ตรวจโฟลเดอร์จริงแล้วเจอไฟล์ที่ audit ภายนอกเจอ **ตรงกัน**
4. test ยืนยันว่า access token ไม่โผล่ใน log/args/ผลลัพธ์
