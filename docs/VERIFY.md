...
#### บล็อก 5 · check · dedupe · export

| # | ทำอะไร | บรรทัดที่ต้องเห็น | ผล | ผู้ทดสอบ / วันที่ |
|---|---|---|---|---|
| 5.1 | สั่ง check | `rb.job: start task ... kind=check` | ⬜ | |
| 5.2 | สั่ง dedupe | `rb.job: start task ... kind=dedupe` | ⬜ | |
| 5.3 | สั่ง export | `rb.job: start task ... kind=export` | ⬜ | |
| 5.4 | งานพวกนี้ต้องขึ้นในหน้า Jobs และในประวัติ | มีแถวใน `job_run` | ⬜ | |
| 5.5 | teldrive → ปุ่ม dedupe/cleanup ต้องปิดอยู่ (V-09) | ไม่มี `start task` เกิดขึ้น | ⬜ | |
