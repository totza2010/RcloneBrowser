...
#### บล็อก 2 · ค้นหา remote และเวอร์ชัน

| # | ทำอะไร | บรรทัดที่ต้องเห็น | ผล | ผู้ทดสอบ / วันที่ |
|---|---|---|---|---|
| 2.1 | เปิดโปรแกรม | `rb.remote: listing remotes with <path>` แล้ว `found n=` | ✅ **PASS** | AI / 2026-08-26 · `found n= 6` |
| 2.2 | เพิ่ม remote ใน rclone config แล้วกด refresh | `found n=` เพิ่มขึ้นหนึ่ง | ⬜ | |
| 2.3 | ตั้ง path rclone ผิด | `rb.remote: refused` พร้อมเหตุผล + กล่องข้อความ | ⬜ | |
| 2.4 | remote ที่ **type** มี `:` | ไม่หายไป (ของเดิมหาย — ดูหมายเหตุ) | ⬜ | ครอบด้วย `test_remote_registry` แล้ว |
| 2.5 | เปิดโปรแกรม | `rb.remote: rclone version "x" at "<path>"` | ✅ **PASS** | AI / 2026-08-26 · `1.73.1` |
| 2.6 | config ที่ใส่รหัสผ่านไว้ | ถามรหัส แล้ว list ได้หลังใส่ถูก | ⬜ | |
| 2.7 | rclone เก่ามากจนไม่มี `listremotes` | ขึ้นข้อความให้อัปเกรด ไม่ใช่ "verify path" | ⬜ | |
...
#### บล็อก 3 · สร้างและแก้ task

| # | ทำอะไร | บรรทัดที่ต้องเห็น | ผล | ผู้ทดสอบ / วันที่ |
|---|---|---|---|---|
| 3.1 | สร้าง task ใหม่จาก Transfer dialog | `rb.task: created id= name=` | ⬜ | |
| 3.2 | แก้ task เดิม | `rb.task: updated id=` พร้อมชื่อฟิลด์ที่เปลี่ยน | ⬜ | |
| 3.3 | ลบ task | `rb.task: deleted id=` | ⬜ | |
| 3.4 | สร้างโดยไม่ตั้งชื่อ → ต้องได้ชื่ออัตโนมัติ | `rb.task: auto name=` | ⬜ | |
| 3.5 | เปลี่ยนชื่อ task ที่มีตารางเวลาชี้อยู่ | ชื่อในหน้า scheduler เปลี่ยนตาม และ `(*Sch)` ยังอยู่ | ⬜ | |
| 3.6 | **สร้าง task จาก `TaskBuilder` ใน test โดยไม่มี dialog** | `rb.task: created` ใน test log | ⬜ | |
| 3.7 | task ที่สร้างจาก dialog กับจาก `TaskBuilder` ต้องได้ args เหมือนกันเป๊ะ | เทียบ `getOptions()` ใน test | ⬜ | |
