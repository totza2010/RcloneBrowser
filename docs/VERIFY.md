...
| 3 | ตารางที่ activated ถึงเวลา → เห็น `fired` แล้วตามด้วย `queueing`/`starting now` | ⬜ | | |
| 4 | requestId ใน `fired` ตรงกับ `enqueue request=` ของ `rb.queue` | ⬜ | | |
...
| 7 | งานจบแล้วตารางยังขึ้น "in the queue" → ต้องเห็น `status ignored` อธิบาย | ⬜ | | |

> ข้อ 5 กับ 7 คือสองอาการที่เคยเจอมาแล้วจริง — ใส่ไว้เพื่อให้รอบหน้าตอบได้จาก log
> โดยไม่ต้องเดา

> ⚠️ **ข้อ 3 คือข้อที่ต้องมีคนลองหลังย้ายนาฬิกา (2026-08-25)** การยิงงานย้ายจาก
> `QTimer` ในแต่ละ widget ไปเป็น `SchedulerStore::due()` + นาฬิกาเดียวใน `MainWindow`
> ผมยืนยันได้แค่ว่า**นาฬิกาเดินและไม่ยิงมั่ว** ส่วน**เส้นทางการยิงจริงยังไม่ได้ทดสอบ**
> เพราะตารางของผู้ใช้ตั้งไว้ 22:29 กับ 00:00 ซึ่งรอไม่ได้
>
> **วิธีทดสอบ:** ตั้งตารางใหม่ให้ถึงเวลาในอีก 2 นาที เปิด `RB_DEBUG=1` แล้วดูว่าได้ครบสาย
> `due` → `fired` → `queueing`/`starting now` → `enqueue` (ของ `rb.queue`) โดยใช้
> requestId เดียวกันตลอด
